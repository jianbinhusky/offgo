...
public class MapData<K,V> extends LinkedHashMap<K,V> {
  ...
  public MapData(Generator<net.mindview.util.Pair<K,V>> gen, int quantity) {
    for(int i = 0; i < quantity; i++) {
      net.mindview.util.Pair<K,V> p = gen.next();
      put(p.key, p.value);
    }
  }
  ...
  map(Generator<net.mindview.util.Pair<K,V>> gen, int quantity) {
